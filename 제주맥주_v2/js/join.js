    ...
    .blur(function () {
      ...
      *****************************************/
      if (cv === "") {
        ...
        $(this).siblings(".msg")
          .text("필수입력")
          // 선택한 거 형제들 중 .msg를 찾아서 메시지"필수입력"를 띄워라
          .removeClass("on"); // 클래스 "on"을 빼면 원래글자색(초록글자였다가 다시 빨간글자가 되야하는 경우)
      ...
      ******************************************/
        ...
        if (!vReg(cv, cid)) { // !(NOT) 반대로 들어감(true가 아닐때=false일때)
          // vReg(cv,cid)가 false났을 때 선택한 거 다른형제들 중에서 .msg를 찾아서 메시지"영문자로 시작하는 6~20글자 영문자/숫자
          // 선택한 거 다른형제들 중에서 .msg를 찾아서 메시지"훌륭한 아이디네요~!"를 띄워라"를 띄워라
          $(this).siblings(".msg").text("영문자로 시작하는 6~20글자 영문자/숫자")
            .removeClass("on");
          // 클래스 "on"을 빼면 원래글자색(조건에 안맞게 넣으면)


        } ///// if : 아이디 검사 불통과 /////
        /////// 검사결과가 통과이면 ////////
        else {
          // 선택한 거 다른형제들 중에서 .msg를 찾아서 메시지"멋진 아이디네요!"를 띄워라
          $(this).siblings(".msg")
            .text("멋진 아이디네요!")
            .addClass("on");
          // 클래스 "on"을 넣으면 녹색글자(vReg(cv,cid)가 true일때-조건에 맞게 넣으면)
        } //// else : 아이디 검사 통과 ////
